...
describe('intitialState', () => {
  it('should return correct initial state', () => {
     ...
     return promise.then(state => {
       ...
       expect(state.getIn(['currencies', 'RUB', 'results']).size).to.equal(4)
       expect(state.getIn(['currencies', 'RUB', 'results']).maxBy((v,k) => k).size).to.equal(4)
       expect(state.getIn(['currencies', 'RUB', 'results']).maxBy((v,k) => k).get('result')).not.to.equal(0)
       expect(state.getIn(['currencies', 'RUB', 'results']).minBy((v,k) => k).size).to.equal(4)
       expect(state.getIn(['currencies', 'EUR', 'results']).size).to.equal(4)
       expect(state.getIn(['currencies', 'EUR', 'results']).maxBy((v,k) => k).size).to.equal(4)
       expect(state.getIn(['currencies', 'EUR', 'results']).maxBy((v,k) => k).get('result')).not.to.equal(0)
       expect(state.getIn(['currencies', 'EUR', 'results']).minBy((v,k) => k).size).to.equal(4)
       expect(state.getIn(['currencies', 'USD', 'results']).size).to.equal(4)
       expect(state.getIn(['currencies', 'USD', 'results']).maxBy((v,k) => k).size).to.equal(4)
       expect(state.getIn(['currencies', 'USD', 'results']).maxBy((v,k) => k).get('result')).not.to.equal(0)
       expect(state.getIn(['currencies', 'USD', 'results']).minBy((v,k) => k).size).to.equal(4)
     })
  })
})
